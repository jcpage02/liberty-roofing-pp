...
class Appointments extends Component {
  ...
  addAppointment = () => {
    const {firstName, lastName, address, city, state, phone, email, images, date, type} = this.state
    axios.post('/api/apts',{firstName, lastName, address, city, state, phone, email, images, date, type})
    .then(res => {
...
/////////////////////////////////////// ADD NEW ROW ////////////////////////////////////
    let addBtns = newAppointment ? (
      <div className={dashTblBtnCol}>
        <i className="far fa-save fa-lg" onClick={() => this.addAppointment()} />
      </div>
    ) : null;
    ...
    let addPhone = newAppointment ? (
      <div className={dashTblCell}>
        <input placeholder="Phone #" onChange={e => {this.handleChange('phone', e.target.value)}} />
      </div>
    ) : null;
    ...
    let addImages = newAppointment ? (
      <div className={dashTblCell}>
        <input placeholder="Images" onChange={e => {this.handleChange('images', e.target.value)}} />
      </div>
    ) : null;
    let addDate = newAppointment ? (
      <div className={dashTblCell}>
        <input placeholder="Date" onChange={e => {this.handleChange('date', e.target.value)}} />
      </div>
    ) : null;
    let addType = newAppointment ? (
      <div className={dashTblCell}>
        <input placeholder="Type" onChange={e => {this.handleChange('type', e.target.value)}} />
      </div>
    ) : null;
...
/////////////////////////////////////// ROWS FROM DATABASE ////////////////////////////////////
    let idColumn = ids.map((id, i) => (
      <div className={dashTblBtnCol} key={i}>
        ...
        <i className={dashTblDelBtn} value={id} onClick={() => this.deleteAppointment(i, id)}/>
      </div>
    ));
